                ...
                <div class="checkout-container">
                    ...
                    <div class="checkout_card shadow">
                        ...
                        <div class="d-flex mb-3"
                            ...
                            <h2>₹{{ number_format($course->price / 100) }}</h2>
                            <h2><span class="text-secondary">(1<sup>th</sup> Installment)</span>&nbsp;&nbsp;-
                                ₹{{ number_format(($course->price - $half_price) / 100) }}</h2>
                            <div style="width:100%;height:1px;background:gray;margin-bottom:10px;margin-top:5px"></div>
                            <h2>₹{{ number_format($half_price / 100) }}
                            </h2>
                        </div>
                        <div class="row mb-3">
                            <div class="col-md-12">
                        ...
                        <form action="{{ route('installment.store', $course->slug) }}" method="POST">
                            @csrf
                            <input type="hidden" value="{{ request('batch') }}" name="batch">
                            <input type="hidden" value="{{ $half_price }}" name="course_price">
                            <script src="https://checkout.razorpay.com/v1/checkout.js" data-key="{{ env('RAZORPAY_KEY') }}"
                                                        data-amount="{{ $half_price }}" data-currency="INR" data-buttontext="I Agree and Buy"

    ...
    <div class="container">
        ...
        <div class="row justify-content-center">
            <div class="col-md-10">
                ...
                @endif
                <div class="card">
                    <div class="card-header">{{ __('Update Placement Details') }}</div>
                    <form class="card-body" action="{{route('placement.update',$placement_details->id)}}" method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3">
                            <label for="full_name" class="form-label">Enter Full Name</label>
                            <input type="text" class="form-control" id="full_name" name="full_name"
                                value="{{ $placement_details->full_name }} ">
                            @error('full_name')
                        ...
                        <div class="mb-3">
                            ...
                            value="{{ $placement_details->company_name }} ">
                            @error('company_name')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        <div class="mb-3">
                            ...
                            value="{{ $placement_details->job_title }} ">
                            @error('job_title')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        <div class="mb-3">
                            ...
                            value="{{ $placement_details->location }} ">
                            @error('location')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        <div class="mb-3">
                            ...
                            value="{{ $placement_details->package }} ">
                            @error('package')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>

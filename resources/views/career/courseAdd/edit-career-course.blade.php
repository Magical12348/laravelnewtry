...
@section('content')
    ...
    <section class="container-fluid max_width my-3">
        ...
        @endif
        <form action="{{ route('career.update.course.index', $career) }}" method="post" class="card"
            id="careers-form">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-12 mb-3">
                    ...
                    <select class="form-select" id="name_of_course" name="name_of_course"
                        ...
                        @foreach (config('setting.select_courses') as $s_course)
                            <option {{ $career->name_of_course == $s_course ? 'selected' : '' }}
                ...
                <div class="col-md-12 mb-3" style="display: none" id="other_name_of_course_container">
                    ...
                    <input type="text" class="form-control" id="other_name_of_course" name="other_name_of_course"
                        value="{{ $career->other_name_of_course }}">
                ...
                <div class="col-md-12 mb-3">
                    ...
                    <textarea class="form-control" id="course_description" name="course_description"
                        rows="4">{{ $career->course_description }}</textarea>
                ...
                <div class="col-md-12 mb-3">
                    <label for="syllabus" class="form-label">Syllabus<span style="color:red">*</span>
                        :</label>
                    <textarea class="form-control" id="syllabus" name="syllabus" rows="4">{{ $career->syllabus }}</textarea>
                    @error('syllabus')
                        <small class="text-danger">{{ $message }}</small>
                    @enderror
                </div>
                <div class="col-md-12 mb-3">
                    ...
                    <textarea class="form-control" id="course_keyword" name="course_keyword"
                        rows="2">{{ $career->course_keyword }}</textarea>
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <select class="form-select" id="have_pre_requisite" name="have_pre_requisite"
                        ...
                        <option selected disabled>-select-</option>
                        <option {{ $career->have_pre_requisite == '1' ? 'selected' : '' }} value="1">Yes
                        </option>
                        <option {{ $career->have_pre_requisite == '0' ? 'selected' : '' }} value="0">No
                        </option>
                    </select>
                    @error('have_pre_requisite')
                        <small class="text-danger">{{ $message }}</small>
                ...
                <div class="col-md-8 mb-3">
                    ...
                    <textarea class="form-control" id="tell_pre_requisite" name="tell_pre_requisite"
                        rows="2">{{ $career->tell_pre_requisite }}</textarea>
                </div>
                <div class="col-md-12 mb-3">
                    ...
                    <textarea class="form-control" id="opportunity_after_course" name="opportunity_after_course"
                        rows="2">{{ $career->opportunity_after_course }}</textarea>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select" id="have_mock_test_interview" name="have_mock_test_interview">
                        <option selected disabled>-select-</option>
                        <option {{ $career->have_mock_test_interview == 'mock test' ? 'selected' : '' }}
                            value="mock test">
                            Mock Test</option>
                        <option {{ $career->have_mock_test_interview == 'interview' ? 'selected' : '' }}
                            value="interview">
                            Interview</option>
                        <option {{ $career->have_mock_test_interview == 'both' ? 'selected' : '' }} value="both">Mock
                            Test
                            and Interview</option>
                        <option {{ $career->have_mock_test_interview == 'no' ? 'selected' : '' }} value="no">No
                        </option>
                    </select>
                ...
                <div class="col-md-6  mb-3">
                    ...
                    <select class="form-select" id="conduct_lecture" name="conduct_lecture">
                        <option selected disabled>-select-</option>
                        <option {{ $career->conduct_lecture == '1' ? 'selected' : '' }} value="1">Yes
                        </option>
                        <option {{ $career->conduct_lecture == '0' ? 'selected' : '' }} value="0">No
                        </option>
                    </select>
